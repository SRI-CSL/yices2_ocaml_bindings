(** Low-level Ctypes bindings and type definitions for the Yices C API. *)
open Ctypes
open Ctypes_zarith
open Unsigned
open Signed
...
module BaseTypes = struct
  ...
  type error_code = 
    ...
    | `CTX_EF_ASSERTIONS_CONTAIN_UF
    | `CTX_EF_NOT_EXISTS_FORALL
    | `CTX_EF_HIGH_ORDER_VARS
    | `CTX_EF_INTERNAL_ERROR
    | `CTX_FORMULA_NOT_IDL
    | `CTX_FORMULA_NOT_RDL
...
{v
yices_build_arch; yices_build_mode; yices_build_date
v}
  ...
  val yices_build_arch : char ptr ptr checkable
  val yices_build_mode : char ptr ptr checkable
  val yices_build_date : char ptr ptr checkable
  ...
  val yices_has_mcsat : unit -> bool_t checkable
  ...
  (* Check whether the library was compiled in THREAD_SAFE mode.
   ...
   - return 1 if yes, 0 if no *)
  val yices_is_thread_safe : unit -> bool_t checkable
  ...
  val yices_free_string : char ptr -> unit
  ...
  (* By default, when Yices runs out of memory, it
   ...
   {v
   yices_set_out_of_mem_callback cb
   v}
  ...
  (* Get the last error code
   ...
   v}
  *)
  val yices_error_code : unit -> error_code_t
  ...
  (* Get the last error report
   ...
   v}
  *)
  val yices_error_report : unit -> error_report_t ptr
  ...
  *)
  ...
  (* Print an error message on file descriptor fd.  This converts the current error
   ...
   If there's an error, errno, perror, and friends can be used for diagnostic. *)
  val yices_print_error_fd : sint -> unit_t checkable
  ...
  (* Build a string from the current error code + error report structure.
   ...
   yices_free_string. *)
  val yices_error_string : unit -> char ptr checkable

  (** {2 VECTORS OF TERMS AND TYPES} *)
  ...
  (* Some functions in the API return arrays of terms or types
   ...
   - v.data = the array proper: the elements are stored in
              ...
              v.data0 .... v.datan-1 where n = v.size.
...
{v
yices_init_term_vector v; yices_init_type_vector v
v}
  *)
  val yices_init_term_vector : term_vector_t ptr -> unit
  val yices_init_type_vector : type_vector_t ptr -> unit

  (* Delete vector v
   
{v
yices_delete_term_vector v; yices_delete_type_vector v
v}
  *)
  val yices_delete_term_vector : term_vector_t ptr -> unit
  val yices_delete_type_vector : type_vector_t ptr -> unit

  (* Reset: empty the vector (reset size to 0)
   
{v
yices_reset_term_vector v; yices_reset_type_vector v
v}
  *)
  val yices_reset_term_vector : term_vector_t ptr -> unit
  val yices_reset_type_vector : type_vector_t ptr -> unit
...
{v
yices_bool_type (); yices_int_type (); yices_real_type ()
v}
  *)
  val yices_bool_type : unit -> type_t checkable
  val yices_int_type  : unit -> type_t checkable
  val yices_real_type : unit -> type_t checkable
  ...
  (* New scalar type of given cardinality.
   ...
   If card = 0, set error report to
     code = POS_INT_REQUIRED
     badval = size *)
  val yices_new_scalar_type : uint -> type_t checkable
  ...
  (* New uninterpreted type. No error report.
   ...
   v}
  *)
  val yices_new_uninterpreted_type : unit -> type_t checkable

  (* Tuple type tau0 x ... x taun-1.
   
   {v
   yices_tuple_type n taus
   v}
   ...
   v}
  *)
  val yices_tuple_type : uint -> type_t ptr -> type_t checkable

  (* Variants: for small arity
   
{v
yices_tuple_type1 tau; yices_tuple_type2 tau sigma; yices_tuple_type3 tau sigma rho
v}
  ...
  val yices_tuple_type1 : type_t -> type_t checkable
  val yices_tuple_type2 : type_t -> type_t -> type_t checkable
  val yices_tuple_type3 : type_t -> type_t -> type_t -> type_t checkable

  (* Function type: dom0 ... domn-1] -> range
   
   {v
   yices_function_type n taus tau
   v}
   ...
   v}
  *)
  val yices_function_type : uint -> type_t ptr -> type_t -> type_t checkable

  (* Variants for small arity:
   
{v
yices_function_type1 tau sigma; yices_function_type2 tau sigma rho; yices_function_type3 tau sigma rho delta
v}
  ...
  val yices_function_type1 : type_t -> type_t -> type_t checkable
  val yices_function_type2 : type_t -> type_t -> type_t -> type_t checkable
  val yices_function_type3 : type_t -> type_t -> type_t -> type_t -> type_t checkable
...
{v
yices_type_is_bool tau; yices_type_is_int tau; yices_type_is_real tau; yices_type_is_arithmetic tau; yices_type_is_bitvector tau; yices_type_is_tuple tau; yices_type_is_function tau; yices_type_is_scalar tau; yices_type_is_uninterpreted tau
v}
   ...
   and set the error report:
     code = INVALID_TYPE
     type1 = tau *)
  val yices_type_is_bool       : type_t -> bool_t checkable
  val yices_type_is_int        : type_t -> bool_t checkable
  val yices_type_is_real       : type_t -> bool_t checkable
  val yices_type_is_arithmetic : type_t -> bool_t checkable
  val yices_type_is_bitvector  : type_t -> bool_t checkable
  val yices_type_is_tuple      : type_t -> bool_t checkable
  val yices_type_is_function   : type_t -> bool_t checkable
  val yices_type_is_scalar     : type_t -> bool_t checkable
  val yices_type_is_uninterpreted : type_t -> bool_t checkable

  (* Check whether tau is a subtype of sigma
   
   {v
   yices_test_subtype tau sigma
   v}
   ...
   and sets the error report:
     code = INVALID_TYPE
     type1 = tau or sigma *)
  val yices_test_subtype : type_t -> type_t -> bool_t checkable

  (* Check whether tau and sigma are compatible
   
   {v
   yices_compatible_types tau sigma
   v}
   ...
   sets the error report:
     code = INVALID_TYPE
     type1 = tau or sigma *)
  val yices_compatible_types : type_t -> type_t -> bool_t checkable

  (* Number of bits for type tau
   
   {v
   yices_bvtype_size tau
   v}
   ...
   v}
  *)
  val yices_bvtype_size : type_t -> uint_t checkable

  (* Cardinality of a scalar type
   
   {v
   yices_scalar_type_card tau
   v}
   ...
   v}
  *)
  val yices_scalar_type_card : type_t -> uint_t checkable

  (* Number of children of type tau
   
   {v
   yices_type_num_children tau
   v}
   ...
   v}
  *)
  val yices_type_num_children : type_t -> sint_t checkable

  (* i-th child of type tau.
   
   {v
   yices_type_child tau i
   v}
   ...
   v}
  *)
  val yices_type_child : type_t -> sint -> type_t checkable

  (* Collect all the children of type tau in vector *v
   
   {v
   yices_type_children tau v
   v}
   ...
   v}
  *)
  val yices_type_children : type_t -> type_vector_t ptr -> unit_t checkable
...
{v
yices_true (); yices_false ()
v}
  *)
  val yices_true : unit -> term_t checkable
  val yices_false : unit -> term_t checkable

  (* Constant of type tau and id = index
   
   {v
   yices_constant tau i
   v}
   ...
   v}
  *)
  val yices_constant : type_t -> sint -> term_t checkable

  (* Uninterpreted term of type tau
   
   {v
   yices_new_uninterpreted_term tau
   v}
   ...
   v}
  *)
  val yices_new_uninterpreted_term : type_t -> term_t checkable

  (* Variable of type tau. This creates a new variable.
   
   {v
   yices_new_variable tau
   v}
   ...
   v}
  *)
  val yices_new_variable : type_t -> term_t checkable

  (* Application of an uninterpreted function to n arguments.
   
   {v
   yices_application t n ts
   v}
   ...
   v}
  *)
  val yices_application : term_t -> uint -> term_t ptr -> term_t checkable

  (* Variants for small arity:
   
{v
yices_application1 t u; yices_application2 t u v; yices_application3 t u v w
v}
  ...
  val yices_application1 : term_t -> term_t -> term_t checkable
  val yices_application2 : term_t -> term_t -> term_t -> term_t checkable
  val yices_application3 : term_t -> term_t -> term_t -> term_t -> term_t checkable
  ...
  (* if-then-else
   ...
   v}
  *)
  val yices_ite : term_t -> term_t -> term_t -> term_t checkable

  (* Equality (= left right)
   
{v
yices_eq t u; yices_neq t u
v}
   ...
   v}
  *)
  val yices_eq : term_t -> term_t -> term_t checkable
  val yices_neq : term_t -> term_t -> term_t checkable
  ...
  (* (not arg)
   ...
   v}
  *)
  val yices_not : term_t -> term_t checkable

  (* (or  arg0 ... argn-1)
   
{v
yices_or n ts; yices_and n ts; yices_xor n ts
v}
   ...
   v}
  *)
  val yices_or : uint -> term_t ptr -> term_t checkable
  val yices_and : uint -> term_t ptr -> term_t checkable
  val yices_xor : uint -> term_t ptr -> term_t checkable

  (* Variants of or/and/xor with 2 or 3 arguments
   
{v
yices_or2 t u; yices_and2 t u; yices_xor2 t u; yices_or3 t u v; yices_and3 t u v; yices_xor3 t u v
v}
  *)
  val yices_or2 : term_t -> term_t -> term_t checkable
  val yices_and2 : term_t -> term_t -> term_t checkable
  val yices_xor2 : term_t -> term_t -> term_t checkable
  val yices_or3 : term_t -> term_t -> term_t -> term_t checkable
  val yices_and3 : term_t -> term_t -> term_t -> term_t checkable
  val yices_xor3 : term_t -> term_t -> term_t -> term_t checkable

  (* (iff left right)
   
{v
yices_iff t u; yices_implies t u
v}
   ...
   v}
  *)
  val yices_iff : term_t -> term_t -> term_t checkable
  val yices_implies : term_t -> term_t -> term_t checkable
  ...
  (* Tuple constructor
   ...
   v}
  *)
  val yices_tuple : uint -> term_t ptr -> term_t checkable

  (* Variants for n=2 or n=3
   
{v
yices_pair t u; yices_triple t u v
v}
   
   - same error reports as yices_tuple if arg1, arg2, or arg3 is invalid *)
  val yices_pair : term_t -> term_t -> term_t checkable
  val yices_triple : term_t -> term_t -> term_t -> term_t checkable
  ...
  (* Tuple projection
   ...
   v}
  *)
  val yices_select : uint -> term_t -> term_t checkable

  (* Tuple update: replace component i of tuple by new_v
   
   {v
   yices_tuple_update t n u
   v}
   ...
   v}
  *)
  val yices_tuple_update : term_t -> uint -> term_t -> term_t checkable

  (* Function update
   
   {v
   yices_update t n ts u
   v}
   ...
   v}
  *)
  val yices_update : term_t -> uint -> term_t ptr -> term_t -> term_t checkable

  (* Variants of yices_update for small n
   
{v
yices_update1 t u v; yices_update2 t u v w; yices_update3 t u v w x
v}
   
   - fun must be a function of arity 1, 2, or 3
   - the arguments are given as arg1, arg2, arg3 instead of an array arg[] *)
  val yices_update1 : term_t -> term_t -> term_t -> term_t checkable
  val yices_update2 : term_t -> term_t -> term_t -> term_t -> term_t checkable
  val yices_update3 : term_t -> term_t -> term_t -> term_t -> term_t -> term_t checkable
  ...
  (* Distinct
   ...
   v}
  *)
  val yices_distinct : uint -> term_t ptr -> term_t checkable

  (* Quantified terms
   
{v
yices_forall n ts t; yices_exists n ts t
v}
   ...
   v}
  *)
  val yices_forall : uint -> term_t ptr -> term_t -> term_t checkable
  val yices_exists : uint -> term_t ptr -> term_t -> term_t checkable
  ...
  *)
  val yices_lambda : uint -> term_t ptr -> term_t -> term_t checkable
  ...
  (* Zero: no error
   ...
   v}
  *)
  val yices_zero : unit -> term_t checkable

  (* Integer constants
   
{v
yices_int32 i; yices_int64 i
v}
  *)
  val yices_int32 : sint -> term_t checkable
  val yices_int64 : long -> term_t checkable

  (* Rational constants
   
{v
yices_rational32 i n; yices_rational64 i n
v}
...
v}
  ...
  (* Convert a string to a rational or integer term.
   ...
   Error report:
     ...
     v}
  *)
  val yices_parse_rational : char ptr -> term_t checkable
  ...
  (* Convert a string in floating point format to a rational
   ...
   v}
  *)
  val yices_parse_float : char ptr -> term_t checkable
...
{v
yices_add t u; yices_sub t u; yices_neg t; yices_mul t u; yices_square t; yices_power t n
v}
  ...
  *)
  val yices_add : term_t -> term_t -> term_t checkable
  val yices_sub : term_t -> term_t -> term_t checkable
  val yices_neg : term_t -> term_t checkable
  val yices_mul : term_t -> term_t -> term_t checkable
  val yices_square : term_t -> term_t checkable
  val yices_power : term_t -> uint -> term_t checkable
  ...
  (* Sum of n arithmetic terms t0 ... tn-1
   ...
   v}
  *)
  val yices_sum : uint -> term_t ptr -> term_t checkable
  ...
  (* Product of n arithmetic terms t0 ... tn-1
   ...
   v}
  *)
  val yices_product : uint -> term_t ptr -> term_t checkable
  ...
  (* Division:  t1/t2
   ...
   v}
  *)
  val yices_division : term_t -> term_t -> term_t checkable

  (* Integer division and modulo
   
{v
yices_idiv t u; yices_imod t u
v}
   ...
   v}
  *)
  val yices_idiv : term_t -> term_t -> term_t checkable
  val yices_imod : term_t -> term_t -> term_t checkable

  (* Divisibility test:
   
   {v
   yices_divides_atom t u
   v}
   ...
   v}
  *)
  val yices_divides_atom : term_t -> term_t -> term_t checkable
  ...
  *)
  val yices_is_int_atom : term_t -> term_t checkable

  (* Absolute value, floor, ceiling
   
{v
yices_abs t; yices_floor t; yices_ceil t
v}
...
{v
yices_poly_int32 n p ts; yices_poly_int64 n p ts
v}
   
   - a and t must both be arrays of size n *)
  val yices_poly_int32 : uint -> sint ptr -> term_t ptr -> term_t checkable
  val yices_poly_int64 : uint -> long ptr -> term_t ptr -> term_t checkable

  (* Polynomial with rational coefficients
   
{v
yices_poly_rational32 n p q ts; yices_poly_rational64 n p q ts
v}
...
{v
yices_poly_mpz n p ts; yices_poly_mpq n p ts
v}
     - the rationals q0 ... n-1 must all be canonicalized *)
...
{v
yices_arith_eq_atom t u; yices_arith_neq_atom t u; yices_arith_geq_atom t u; yices_arith_leq_atom t u; yices_arith_gt_atom t u; yices_arith_lt_atom t u
v}
   ...
   v}
  *)
  val yices_arith_eq_atom : term_t -> term_t -> term_t checkable
     val yices_arith_neq_atom : term_t -> term_t -> term_t checkable
     val yices_arith_geq_atom : term_t -> term_t -> term_t checkable
     val yices_arith_leq_atom : term_t -> term_t -> term_t checkable
     val yices_arith_gt_atom : term_t -> term_t -> term_t checkable
     val yices_arith_lt_atom : term_t -> term_t -> term_t checkable

  (* Comparison with 0:
   
{v
yices_arith_eq0_atom t; yices_arith_neq0_atom t; yices_arith_geq0_atom t; yices_arith_leq0_atom t; yices_arith_gt0_atom t; yices_arith_lt0_atom t
v}
   ...
   v}
  *)
  val yices_arith_eq0_atom : term_t -> term_t checkable
  val yices_arith_neq0_atom : term_t -> term_t checkable
  val yices_arith_geq0_atom : term_t -> term_t checkable
  val yices_arith_leq0_atom : term_t -> term_t checkable
  val yices_arith_gt0_atom : term_t -> term_t checkable
  val yices_arith_lt0_atom : term_t -> term_t checkable
...
{v
yices_bvconst_uint32 n m; yices_bvconst_uint64 n m; yices_bvconst_int32 n i; yices_bvconst_int64 n i; yices_bvconst_mpz n p
v}
   ...
   v}
  *)
  val yices_bvconst_uint32 : uint -> uint -> term_t checkable
...
{v
yices_bvconst_zero n; yices_bvconst_one n; yices_bvconst_minus_one n
v}
   ...
   v}
  *)
  val yices_bvconst_zero : uint -> term_t checkable
  val yices_bvconst_one : uint -> term_t checkable
  val yices_bvconst_minus_one : uint -> term_t checkable

  (* Construction from an integer array
   
   {v
   yices_bvconst_from_array n p
   v}
  ...
  *)
  ...
  (* Parsing from a string of characters '0' and '1'
   ...
   v}
  *)
  val yices_parse_bvbin : char ptr -> term_t checkable
  ...
  (* Parsing from a hexadecimal string
   ...
   v}
  *)
  val yices_parse_bvhex : char ptr -> term_t checkable
...
{v
yices_bvadd t u; yices_bvsub t u; yices_bvneg t; yices_bvmul t u; yices_bvsquare t; yices_bvpower t n; yices_bvdiv t u; yices_bvrem t u; yices_bvsdiv t u; yices_bvsrem t u; yices_bvsmod t u; yices_bvnot t; yices_bvnand t u; yices_bvnor t u; yices_bvxnor t u; yices_bvshl t u; yices_bvlshr t u; yices_bvashr t u
v}
  ...
  *)
  val yices_bvadd : term_t -> term_t -> term_t checkable
  val yices_bvsub : term_t -> term_t -> term_t checkable
  val yices_bvneg : term_t -> term_t checkable
  val yices_bvmul : term_t -> term_t -> term_t checkable
  val yices_bvsquare : term_t -> term_t checkable
  val yices_bvpower : term_t -> uint -> term_t checkable
  val yices_bvdiv  : term_t -> term_t -> term_t checkable
  val yices_bvrem  : term_t -> term_t -> term_t checkable
  val yices_bvsdiv : term_t -> term_t -> term_t checkable
  val yices_bvsrem : term_t -> term_t -> term_t checkable
  val yices_bvsmod : term_t -> term_t -> term_t checkable
  val yices_bvnot  : term_t -> term_t checkable
  val yices_bvnand : term_t -> term_t -> term_t checkable
  val yices_bvnor  : term_t -> term_t -> term_t checkable
  val yices_bvxnor : term_t -> term_t -> term_t checkable
  val yices_bvshl  : term_t -> term_t -> term_t checkable
  val yices_bvlshr : term_t -> term_t -> term_t checkable
  val yices_bvashr : term_t -> term_t -> term_t checkable

  (* Bitvector and/or/xor
   
{v
yices_bvand n ts; yices_bvor n ts; yices_bvxor n ts; yices_bvand2 t u; yices_bvor2 t u; yices_bvxor2 t u; yices_bvand3 t u v; yices_bvor3 t u v; yices_bvxor3 t u v
v}
  ...
  *)
  val yices_bvand  : uint -> term_t ptr -> term_t checkable
  val yices_bvor   : uint -> term_t ptr -> term_t checkable
  val yices_bvxor  : uint -> term_t ptr -> term_t checkable
  val yices_bvand2 : term_t -> term_t -> term_t checkable
  val yices_bvor2  : term_t -> term_t -> term_t checkable
  val yices_bvxor2 : term_t -> term_t -> term_t checkable
  val yices_bvand3 : term_t -> term_t -> term_t -> term_t checkable
  val yices_bvor3  : term_t -> term_t -> term_t -> term_t checkable
  val yices_bvxor3 : term_t -> term_t -> term_t -> term_t checkable
  ...
  (* Sum of n bitvector terms t0 ... tn-1
   ...
   v}
  *)
  val yices_bvsum  : uint -> term_t ptr -> term_t checkable

  (* Product of n bitvector terms t0 ... tn-1
   
   {v
   yices_bvproduct n ts
   v}
   ...
   v}
  *)
  val yices_bvproduct : uint -> term_t ptr -> term_t checkable

  (* Shift or rotation by an integer constant n
   
{v
yices_shift_left0 t n; yices_shift_left1 t n; yices_shift_right0 t n; yices_shift_right1 t n; yices_ashift_right t n; yices_rotate_left t n; yices_rotate_right t n
v}
   ...
   v}
  *)
  val yices_shift_left0  : term_t -> uint -> term_t checkable
  val yices_shift_left1  : term_t -> uint -> term_t checkable
  val yices_shift_right0 : term_t -> uint -> term_t checkable
  val yices_shift_right1 : term_t -> uint -> term_t checkable
  val yices_ashift_right : term_t -> uint -> term_t checkable
  val yices_rotate_left  : term_t -> uint -> term_t checkable
  val yices_rotate_right : term_t -> uint -> term_t checkable

  (* Extract a subvector of t
   
   {v
   yices_bvextract t n m
   v}
   ...
   v}
  *)
  val yices_bvextract : term_t -> uint -> uint -> term_t checkable
  ...
  (* Concatenation
   ...
   v}
  *)
  val yices_bvconcat2 : term_t -> term_t -> term_t checkable
  ...
  (* General form of concatenation: the input is an array of n bitvector terms
   ...
   v}
  *)
  val yices_bvconcat : uint -> term_t ptr -> term_t checkable
  ...
  (* Repeated concatenation:
   ...
   v}
  *)
  val yices_bvrepeat : term_t -> uint -> term_t checkable

  (* Sign extension
   
   {v
   yices_sign_extend t n
   v}
   ...
   v}
  *)
  val yices_sign_extend : term_t -> uint -> term_t checkable

  (* Zero extension
   
   {v
   yices_zero_extend t n
   v}
   ...
   v}
  *)
  val yices_zero_extend : term_t -> uint -> term_t checkable

  (* AND-reduction:
   
{v
yices_redand t; yices_redor t
v}
   ...
   v}
  *)
  val yices_redand : term_t -> term_t checkable
  val yices_redor : term_t -> term_t checkable
  ...
  (* Bitwise equality comparison: if t1 and t2 are bitvectors of size n,
   ...
   v}
  *)
  val yices_redcomp : term_t -> term_t -> term_t checkable
  ...
  (* Convert an array of boolean terms arg[0 ... n-1 into
   ...
   v}
  *)
  val yices_bvarray : uint -> term_t ptr -> term_t checkable

  (* Extract bit i of vector t (as a boolean)
   
   {v
   yices_bitextract t n
   v}
   ...
   v}
  *)
  val yices_bitextract : term_t -> uint -> term_t checkable
...
{v
yices_bveq_atom t u; yices_bvneq_atom t u
v}
  *)
  val yices_bveq_atom : term_t -> term_t -> term_t checkable
  val yices_bvneq_atom : term_t -> term_t -> term_t checkable

  (* Unsigned inequalities
   
{v
yices_bvge_atom t u; yices_bvgt_atom t u; yices_bvle_atom t u; yices_bvlt_atom t u
v}
  *)
  val yices_bvge_atom : term_t -> term_t -> term_t checkable
  val yices_bvgt_atom : term_t -> term_t -> term_t checkable
  val yices_bvle_atom : term_t -> term_t -> term_t checkable
  val yices_bvlt_atom : term_t -> term_t -> term_t checkable

  (* Signed inequalities
   
{v
yices_bvsge_atom t u; yices_bvsgt_atom t u; yices_bvsle_atom t u; yices_bvslt_atom t u
v}
  *)
  val yices_bvsge_atom : term_t -> term_t -> term_t checkable
  val yices_bvsgt_atom : term_t -> term_t -> term_t checkable
  val yices_bvsle_atom : term_t -> term_t -> term_t checkable
  val yices_bvslt_atom : term_t -> term_t -> term_t checkable
...
{v
yices_parse_type s; yices_parse_term s
v}
  ...
  val yices_parse_type : char ptr -> type_t checkable
  val yices_parse_term : char ptr -> term_t checkable
  ...
  *)
  val yices_subst_term : uint -> term_t ptr -> term_t ptr -> term_t -> term_t checkable

  (* Apply a substitution to m terms in parallel
   
   {v
   yices_subst_term_array n ts us m vs
   v}
...
{v
yices_set_type_name tau s; yices_set_term_name t s
v}
  ...
  val yices_set_type_name : type_t -> char ptr -> unit_t checkable
  val yices_set_term_name : term_t -> char ptr -> unit_t checkable

  (* Remove the current mapping of name
   
{v
yices_remove_type_name s; yices_remove_term_name s
v}
  ...
  val yices_remove_type_name : char ptr -> unit
  val yices_remove_term_name : char ptr -> unit

  (* Get type or term of the given name
   
{v
yices_get_type_by_name s; yices_get_term_by_name s
v}
   
   - return NULL_TYPE or NULL_TERM if there's no type or term with that name *)
  val yices_get_type_by_name : char ptr -> type_t checkable
  val yices_get_term_by_name : char ptr -> term_t checkable

  (* Remove the base name of a type tau or of a term t.
   
{v
yices_clear_type_name tau; yices_clear_term_name t
v}
  ...
  val yices_clear_type_name : type_t -> unit_t checkable
  val yices_clear_term_name : term_t -> unit_t checkable

  (* Get the base name of a term or type
   
{v
yices_get_type_name tau; yices_get_term_name t
v}
  ...
  val yices_get_type_name : type_t -> char ptr checkable
  val yices_get_term_name : term_t -> char ptr checkable
  ...
  (* Get the type of term t
   ...
   and sets the error report:
     code = INVALID_TERM
     term1 = t *)
  val yices_type_of_term : term_t -> type_t checkable

  (* Check the type of a term t:
   
{v
yices_term_is_bool t; yices_term_is_int t; yices_term_is_real t; yices_term_is_arithmetic t; yices_term_is_bitvector t; yices_term_is_tuple t; yices_term_is_function t; yices_term_is_scalar t
v}
  ...
  val yices_term_is_bool : term_t -> bool_t checkable
  val yices_term_is_int  : term_t -> bool_t checkable
  val yices_term_is_real : term_t -> bool_t checkable
  val yices_term_is_arithmetic : term_t -> bool_t checkable
  val yices_term_is_bitvector  : term_t -> bool_t checkable
  val yices_term_is_tuple    : term_t -> bool_t checkable
  val yices_term_is_function : term_t -> bool_t checkable
  val yices_term_is_scalar   : term_t -> bool_t checkable

  (* Size of a bitvector term (i.e., number of bits)
   
   {v
   yices_term_bitsize t
   v}
...
{v
yices_term_is_atomic t; yices_term_is_composite t; yices_term_is_projection t; yices_term_is_sum t; yices_term_is_bvsum t; yices_term_is_product t
v}
   ...
   and set the error report:
      code = INVALID_TERM
      term1 = t *)
  val yices_term_is_atomic : term_t -> bool_t checkable
  val yices_term_is_composite : term_t -> bool_t checkable
  val yices_term_is_projection : term_t -> bool_t checkable
  val yices_term_is_sum : term_t -> bool_t checkable
  val yices_term_is_bvsum : term_t -> bool_t checkable
  val yices_term_is_product : term_t -> bool_t checkable

  (* Constructor of term t:
   
   {v
   yices_term_constructor t
   v}
   ...
   (i.e., YICES_CONSTRUCTOR_ERROR) and sets the error report.
      code = INVALID_TERM
      term1 = t *)
  val yices_term_constructor : term_t -> term_constructor_t checkable

  (* Number of children of term t
   
   {v
   yices_term_num_children t
   v}
  ...
  val yices_term_num_children : term_t -> sint_t checkable

  (* Get i-th child of a composite term
   
   {v
   yices_term_child t i
   v}
  ...
  *)
  val yices_term_child : term_t -> sint -> term_t checkable

  (* Collect all the children of composite term t in vector *v
   
   {v
   yices_term_children t v
   v}
...
{v
yices_proj_index t; yices_proj_arg t
v}
   ...
   if t is not a projection
      code = INVALID_TERM_OP
  *)
  val yices_proj_index : term_t -> sint_t checkable
  val yices_proj_arg : term_t -> term_t checkable

  (* Value of a constant term:
   
{v
yices_bool_const_value t p; yices_bv_const_value t p; yices_scalar_const_value t p; yices_rational_const_value t p
v}
...
{v
yices_sum_component t i p ts; yices_bvsum_component t i p ts
v}
  ...
  *)
  ...
  (* Component of power product t
   ...
   {v
   yices_product_component t i ts p
   v}
   ...
   if t is not of the right kind or i is invalid
      code = INVALID_TERM_OP
  *)
  val yices_product_component : term_t -> sint -> term_t ptr -> uint ptr -> unit_t checkable
...
{v
yices_num_terms (); yices_num_types ()
v}
   and types currently stored in the internal data structures. *)
  val yices_num_terms : unit -> uint
  val yices_num_types : unit -> uint

  (* Reference counting support:
   
{v
yices_incref_term t; yices_decref_term t; yices_incref_type tau; yices_decref_type tau
v}
  ...
  *)
  val yices_incref_term : term_t -> unit_t checkable
  val yices_decref_term : term_t -> unit_t checkable
  val yices_incref_type : type_t -> unit_t checkable
  val yices_decref_type : type_t -> unit_t checkable

  (* The following functions give the number of terms and types
   
{v
yices_num_posref_terms (); yices_num_posref_types ()
v}
   
   that have a positive reference count. They return 0 if
   no call to yices_incref_term or yices_incref_type has been
   made. *)
  val yices_num_posref_terms : unit -> uint
  val yices_num_posref_types : unit -> uint

  (* Call the garbage collector.
   
   {v
   yices_garbage_collect ts n taus m b
   v}
  ...
  val yices_new_config  : unit -> ctx_config_t ptr checkable
  ...
  (* Deletion
   ...
   v}
  *)
  val yices_free_config : ctx_config_t ptr -> unit
  (* Set a configuration parameter:
   
   {v
   yices_set_config p s s2
   v}
  ...
  *)
  val yices_set_config : ctx_config_t ptr -> char ptr -> char ptr -> unit_t checkable

  (* Set config to a default solver type or solver combination for the given logic
   
   {v
   yices_default_config_for_logic p s
   v}
  ...
  *)
  val yices_default_config_for_logic : ctx_config_t ptr -> char ptr -> unit_t checkable
  ...
  val yices_new_context    : ctx_config_t ptr -> context_t ptr checkable
  ...
  (* Deletion
   ...
   v}
  *)
  val yices_free_context   : context_t ptr -> unit

  (* Get status: return the context's status flag
   
   {v
   yices_context_status ctx
   v}
   ...
   - return one of the codes defined in yices_types.h,
     ...
     namely one of the constants
      ...
      STATUS_INTERRUPTED
  *)
  val yices_context_status : context_t ptr -> smt_status_t
  ...
  val yices_reset_context  : context_t ptr -> unit
  ...
  *)
  val yices_push           : context_t ptr -> unit_t checkable
  ...
  (* Pop: backtrack to the previous backtrack point (i.e., the matching
   ...
   - if there's no matching push (i.e., the context stack is empty)
     ...
     code = CTX_INVALID_OPERATION
  *)
  val yices_pop            : context_t ptr -> unit_t checkable

  (* Several options determine how much simplification is performed
   
{v
yices_context_enable_option ctx s; yices_context_disable_option ctx s
v}
  ...
  *)
  val yices_context_enable_option : context_t ptr -> char ptr -> unit_t checkable
  val yices_context_disable_option : context_t ptr -> char ptr -> unit_t checkable
  ...
  *)
  val yices_assert_formula : context_t ptr -> term_t -> unit_t checkable

  (* Assert an array of n formulas t[0 ... n-1
   
   {v
   yices_assert_formulas ctx n ts
   v}
  ...
  val yices_assert_formulas : context_t ptr -> uint -> term_t ptr -> unit_t checkable

  (* Check satisfiability:
   
   {v
   yices_check_context ctx param
   v}
  ...
  val yices_check_context : context_t ptr -> param_t ptr -> smt_status_t

  (* Check satisfiability under assumptions.
   
   {v
   yices_check_context_with_assumptions ctx param n ts
   v}
  ...
  (*
   ...
   {v
   yices_check_context_with_model ctx param mdl n ts
   v}
 ...
 (*
  ...
  {v
  yices_check_context_with_model_and_hint ctx param mdl n ts m
  v}
 ...
 model, i.e.
    ...
    tm, ... , tn-1] will be given v_{m}, ... , v_{n-1} values when deciding
 ...
 Since 2.7.0
  *)
  val yices_check_context_with_model_and_hint : context_t ptr -> param_t ptr -> model_t ptr -> uint -> term_t ptr -> uint -> smt_status_t

(*
 
 {v
 yices_mcsat_set_fixed_var_order ctx n ts
 v}
 ...
 If the context does not have the MCSAT solver enabled
   code = CTX_OPERATION_NOT_SUPPORTED
  *)
  val yices_mcsat_set_fixed_var_order : context_t ptr -> uint -> term_t ptr -> smt_status_t

(*
 
 {v
 yices_mcsat_set_initial_var_order ctx n ts
 v}
 ...
 If the context does not have the MCSAT solver enabled
   code = CTX_OPERATION_NOT_SUPPORTED
  *)
  val yices_mcsat_set_initial_var_order : context_t ptr -> uint -> term_t ptr -> smt_status_t

  (*
   
   {v
   yices_check_context_with_interpolation ic param b
   v}
  ...
  *)
  ...
  (* Add a blocking clause: this is intended to help enumerate different models
   ...
   v}
  *)
  val yices_assert_blocking_clause : context_t ptr -> unit_t checkable
  ...
  val yices_stop_search : context_t ptr -> unit
  ...
  (* Return a parameter record initialized with default settings.
   ...
   v}
  *)
  val yices_new_param_record : unit -> param_t ptr checkable
  ...
  (* Set default search parameters for ctx.
   ...
   v}
  *)
  val yices_default_params_for_context : context_t ptr -> param_t ptr -> unit

  (* Set a parameter in record p
   
   {v
   yices_set_param param s s2
   v}
  ...
  *)
  val yices_set_param : param_t ptr -> char ptr -> char ptr -> unit_t checkable
  ...
  *)
  ...
  (*
   ...
   {v
   yices_get_model_interpolant ctx
   v}
   ...
   Since 2.6.4.
  *)
  val yices_get_model_interpolant : context_t ptr -> term_t checkable
  ...
  val yices_get_model : context_t ptr -> bool_t -> model_t ptr checkable
  ...
  (* Delete model mdl
   ...
   v}
  *)
  val yices_free_model : model_t ptr -> unit
  ...
  (*
   ...
   Since 2.6.4.
  *)
  val yices_new_model : unit -> model_t ptr checkable

  (* Build a model from a term-to-term mapping:
   
   {v
   yices_model_from_map n ts us
   v}
   ...
   v}
  *)
  val yices_model_from_map : uint -> term_t ptr -> term_t ptr -> model_t ptr checkable
...
{v
yices_model_set_int32 mdl t i; yices_model_set_int64 mdl t i; yices_model_set_rational32 mdl t i n; yices_model_set_rational64 mdl t i n
v}
...
{v
yices_model_set_bv_int32 mdl t i; yices_model_set_bv_int64 mdl t i; yices_model_set_bv_uint32 mdl t n; yices_model_set_bv_uint64 mdl t n
v}
  ...
  *)
  ...
  (*
   ...
   {v
   yices_model_set_bv_from_array mdl t n p
   v}
  ...
  *)
  ...
  val yices_model_collect_defined_terms : model_t ptr -> term_vector_t ptr -> unit
  ...
  (* Check whether a formula is satisfiable
   ...
   Since 2.6.2.
  *)
  val yices_check_formula :
    term_t -> char ptr -> model_t ptr ptr -> char ptr -> smt_status_t

  (* Check whether n formulas are satisfiable.
   
   {v
   yices_check_formulas ts n s p s2
   v}
  ...
  val yices_check_formulas :
    term_t ptr -> uint -> char ptr -> model_t ptr ptr -> char ptr -> smt_status_t

  (* Check whether the given delegate is supported
   
   {v
   yices_has_delegate s
   v}
  ...
  val yices_has_delegate : char ptr -> bool_t checkable
  ...
  (* Bit-blast then export the CNF to a file
   ...
   {v
   yices_export_formula_to_dimacs t s b status
   v}
  ...
  *)
  ...
  (* Bit-blast n formulas then export the CNF to a file
   ...
   {v
   yices_export_formulas_to_dimacs ts n s b status
   v}
  ...
  *)
  val yices_get_bool_value : model_t ptr -> term_t -> bool_t ptr -> unit_t checkable

  (* Value of arithmetic term t. The value can be returned as an integer, a
   
{v
yices_get_int32_value mdl t p; yices_get_int64_value mdl t p; yices_get_rational32_value mdl t p q; yices_get_rational64_value mdl t p q; yices_get_double_value mdl t p
v}
   ...
   If t's value does not fit in the *val object
     code = EVAL_OVERFLOW
  *)
  val yices_get_int32_value : model_t ptr -> term_t -> sint ptr -> unit_t checkable
  val yices_get_int64_value : model_t ptr -> term_t -> long ptr -> unit_t checkable
  val yices_get_rational32_value : model_t ptr -> term_t -> sint ptr -> uint ptr -> unit_t checkable
  ...
  (*  * Conversion to an algebraic number.
   ...
   {v
   yices_get_algebraic_number_value mdl t p
   v}
  ...
  *)
  ...
  (* Value of term t of uninterpreted or scalar type
   ...
   {v
   yices_get_scalar_value mdl t p
   v}
   ...
   - if t does not have a scalar or uninterpreted type:
     ...
     term1 = t
  *)
  val yices_get_scalar_value : model_t ptr -> term_t -> sint ptr -> unit_t checkable
...
{v
yices_init_yval_vector p; yices_delete_yval_vector p; yices_reset_yval_vector p
v}
  ...
  val yices_init_yval_vector   : yval_vector_t ptr -> unit
  val yices_delete_yval_vector : yval_vector_t ptr -> unit
  val yices_reset_yval_vector  : yval_vector_t ptr -> unit

  (* Value of term t as a node descriptor.
   
   {v
   yices_get_value mdl t v
   v}
   ...
   If the evaluation fails for other reasons:
     code = EVAL_FAILED *)
  val yices_get_value : model_t ptr -> term_t -> yval_t ptr -> unit_t checkable

  (* Queries on the value of a rational node:
   
{v
yices_val_is_int32 mdl v; yices_val_is_int64 mdl v; yices_val_is_rational32 mdl v; yices_val_is_rational64 mdl v; yices_val_is_integer mdl v
v}
  ...
  val yices_val_is_int32      : model_t ptr -> yval_t ptr -> bool_t checkable
  val yices_val_is_int64      : model_t ptr -> yval_t ptr -> bool_t checkable
  val yices_val_is_rational32 : model_t ptr -> yval_t ptr -> bool_t checkable
  val yices_val_is_rational64 : model_t ptr -> yval_t ptr -> bool_t checkable
  val yices_val_is_integer    : model_t ptr -> yval_t ptr -> bool_t checkable


  (* Get the number of bits in a bv constant, the number of components in a tuple,
   
{v
yices_val_bitsize mdl v; yices_val_tuple_arity mdl v; yices_val_mapping_arity mdl v
v}
  ...
  val yices_val_bitsize       : model_t ptr -> yval_t ptr -> uint_t checkable
  val yices_val_tuple_arity   : model_t ptr -> yval_t ptr -> uint_t checkable
  val yices_val_mapping_arity : model_t ptr -> yval_t ptr -> uint_t checkable

  (* Arity of a function node. This function returns 0 if v has tag
   
   {v
   yices_val_function_arity mdl v
   v}
   
   other than YVAL_FUNCTION, otherwise it returns the function's
   arity (i.e., the number of parameters that the function takes). *)
  val yices_val_function_arity : model_t ptr -> yval_t ptr -> uint_t checkable

  (* Type of a function node. This function returns -1 if v has tag
   
   {v
   yices_val_function_type mdl v
   v}
   
   other than YVAL_FUNCTION. Otherwise, it returns the type of the
   object v.
   Since 2.6.2. *)
  val yices_val_function_type : model_t ptr -> yval_t ptr -> type_t checkable

  (* Get the value of a Boolean node v.
   
   {v
   yices_val_get_bool mdl v p
   v}
   ...
   - returns -1 if v does not have tag YVAL_BOOL and sets the error code
     ...
     to YVAL_INVALID_OP. *)
  val yices_val_get_bool : model_t ptr -> yval_t ptr -> bool_t ptr -> unit_t checkable

  (* Get the value of a rational node v
   
{v
yices_val_get_int32 mdl v p; yices_val_get_int64 mdl v p; yices_val_get_rational32 mdl v p q; yices_val_get_rational64 mdl v p q
v}
   ...
   ( *val ) or in ( *num )/( *den ). *)
  val yices_val_get_int32 : model_t ptr -> yval_t ptr -> sint ptr -> unit_t checkable
  val yices_val_get_int64 : model_t ptr -> yval_t ptr -> long ptr -> unit_t checkable
  val yices_val_get_rational32 : model_t ptr -> yval_t ptr -> sint ptr -> uint ptr -> unit_t checkable
  val yices_val_get_rational64 : model_t ptr -> yval_t ptr -> long ptr -> ulong ptr -> unit_t checkable

  (* Value converted to a floating point number
   
   {v
   yices_val_get_double mdl v p
   v}
  *)
...
{v
yices_val_get_mpz mdl v p; yices_val_get_mpq mdl v p
v}
  *)
  ...
  (*  * Export an algebraic number
   ...
   {v
   yices_val_get_algebraic_number mdl v p
   v}
  ...
  *)
  ...
  (* Get the value of a scalar node:
   ...
   {v
   yices_val_get_scalar mdl v p taus
   v}
  ...
  val yices_val_get_scalar : model_t ptr -> yval_t ptr -> sint ptr -> type_t ptr -> unit_t checkable

  (* Expand a tuple node:
   
   {v
   yices_val_expand_tuple mdl v v2
   v}
   ...
   - the children nodes of v are stored in child0 ... n-1
   
   Return code = 0 if v's tag is YVAL_TUPLE.
   Return code = -1 otherwise and the error code is then set to YVAL_INVALID_OP. *)
  val yices_val_expand_tuple : model_t ptr -> yval_t ptr -> yval_t ptr -> unit_t checkable

  (* Expand a function node f
   
   {v
   yices_val_expand_function mdl v v2 p
   v}
   ...
   - the set of mappings for f is stored in vector *v.
     ...
     in v->data0 ... n-1 where n = v->size
   
   Return code = 0 if v's tag is YVAL_FUNCTION.
   Return code = -1 otherwise and the error code is then set to YVAL_INVALID_OP. *)
  val yices_val_expand_function : model_t ptr -> yval_t ptr -> yval_t ptr -> yval_vector_t ptr -> unit_t checkable

  (* Expand a mapping node m
   
   {v
   yices_val_expand_mapping mdl m args value
   v}
   
   - the mapping is of the form x_1 ... x_k -> v where k = yices_val_mapping_arity(mdl, m)
   - args must be an array of size at least k
   - the nodes (x_1 ... x_k) are stored in args0 ... k-1
   
     the node v is stored in value.
   
   Return code = 0 if v's tag is YVAL_MAPPING.
   Return code = -1 otherwise and the error code is then set to YVAL_INVALID_OP. *)
  val yices_val_expand_mapping : model_t ptr -> yval_t ptr -> yval_t ptr -> yval_t ptr -> unit_t checkable
  ...
  (* Check whether f is true in mdl
   ...
   {v
   yices_formula_true_in_model mdl t
   v}
  ...
  *)
  val yices_formula_true_in_model : model_t ptr -> term_t -> bool_t checkable

  (* Check whether f0 ... n-1 are all true in mdl
   
   {v
   yices_formulas_true_in_model mdl n ts
   v}
  ...
  *)
  val yices_formulas_true_in_model : model_t ptr -> uint -> term_t ptr -> bool_t checkable
  ...
  (* Value of term t converted to a constant term val.
   ...
   Error report
     ...
     v}
  *)
  val yices_get_value_as_term : model_t ptr -> term_t -> term_t checkable

  (* Get the values of terms a0 .. n-1 in mdl and convert the values to terms.
   
   {v
   yices_term_array_value mdl n ts us
   v}
  ...
  val yices_term_array_value : model_t ptr -> uint -> term_t ptr -> term_t ptr -> unit_t checkable
  ...
  (* Get the support of a term t in mdl
   ...
   {v
   yices_model_term_support mdl t v
   v}
   ...
   Since 2.6.2.
  *)
  val yices_model_term_support : model_t ptr -> term_t -> term_vector_t ptr -> unit_t checkable

  (* Get the support of terms a0...n-1 in mdl
   
   {v
   yices_model_term_array_support mdl n ts v
   v}
   ...
   Since 2.6.2.
  *)
  val yices_model_term_array_support :
    model_t ptr -> uint -> term_t ptr -> term_vector_t ptr -> unit_t checkable
  ...
  (* Compute an implicant for t in mdl
   ...
   v}
  *)
  val yices_implicant_for_formula : model_t ptr -> term_t -> term_vector_t ptr -> unit_t checkable

  (* Variant: compute an implicant for an array of formulas in mdl.
   
   {v
   yices_implicant_for_formulas mdl n ts v
   v}
  ...
  (* Compute a generalization of mdl for formula t
   ...
   {v
   yices_generalize_model mdl t n ts mode v
   v}
  ...
  val yices_generalize_model : model_t ptr -> term_t -> uint -> term_t ptr -> yices_gen_mode_t -> term_vector_t ptr -> unit_t checkable

  (* Compute a generalization of mdl for the conjunct (a0 /\ ... /\ an-1])
   
   {v
   yices_generalize_model_array mdl n ts m us mode v
   v}
  *)
...
{v
yices_pp_type f tau n m p; yices_pp_term f t n m p
v}
  ...
  val yices_pp_type : FILE.t ptr -> type_t -> uint -> uint -> uint -> unit_t checkable
  val yices_pp_term : FILE.t ptr -> term_t -> uint -> uint -> uint -> unit_t checkable

  (* Pretty print an array of terms:
   
   {v
   yices_pp_term_array f n ts m p q b
   v}
   ...
   set the error report to:
      code = OUTPUT_ERROR
  *)
  val yices_pp_term_array : FILE.t ptr -> uint -> term_t ptr -> uint -> uint -> uint -> bool_t -> unit_t checkable

  (* Print model mdl on FILE f
   
   {v
   yices_print_model f mdl
   v}
  ...
  val yices_print_model : FILE.t ptr -> model_t ptr -> unit

  (* Pretty printing:
   
   {v
   yices_pp_model f mdl n m p
   v}
   ...
   On error:
     ...
     in this case, errno, perror, etc. can be used for diagnostic. *)
  val yices_pp_model : FILE.t ptr -> model_t ptr -> uint -> uint -> uint -> unit_t checkable

  (* Print the values of n terms in  a model
   
   {v
   yices_print_term_values f mdl n ts
   v}
   ...
   Since 2.6.2.
  *)
  val yices_print_term_values :
    FILE.t ptr -> model_t ptr -> uint -> term_t ptr -> unit_t checkable

  (* Pretty print the values of n terms in  a model
   
   {v
   yices_pp_term_values f mdl n ts m p q
   v}
   ...
   Since 2.6.2.
  *)
  val yices_pp_term_values :
    FILE.t ptr -> model_t ptr -> uint -> term_t ptr -> uint -> uint -> uint -> unit_t checkable

  (* Variants of the above functions that use file descriptors rather than file pointers.
   
{v
yices_pp_type_fd i tau n m p; yices_pp_term_fd i t n m p; yices_pp_term_array_fd i n ts m p q b; yices_print_model_fd i mdl; yices_pp_model_fd i mdl n m p
v}
  ...
  val yices_pp_type_fd : sint -> type_t -> uint -> uint -> uint -> unit_t checkable
  val yices_pp_term_fd : sint -> term_t -> uint -> uint -> uint -> unit_t checkable
  val yices_pp_term_array_fd : sint -> uint -> term_t ptr -> uint -> uint -> uint -> bool_t -> unit_t checkable
  val yices_print_model_fd : sint -> model_t ptr -> unit_t checkable
  val yices_pp_model_fd : sint -> model_t ptr -> uint -> uint -> uint -> unit_t checkable
  (* Since 2.6.2.
   
   {v
   yices_print_term_values_fd i mdl n ts
   v}
  *)
  val yices_print_term_values_fd : sint -> model_t ptr -> uint -> term_t ptr -> unit_t checkable
  (* Since 2.6.2.
   
   {v
   yices_pp_term_values_fd i mdl n ts m p q
   v}
  *)
  val yices_pp_term_values_fd :
    sint -> model_t ptr -> uint -> term_t ptr -> uint -> uint -> uint -> unit_t checkable

  (* Convert type tau or term t to a string using the pretty printer.
   
{v
yices_type_to_string tau n m p; yices_term_to_string t n m p
v}
   ...
   - possible error report for yices_term_to_string
      ...
      code = INVALID_TERM
      term1 = t *)
  val yices_type_to_string : type_t -> uint -> uint -> uint -> char ptr checkable
  val yices_term_to_string : term_t -> uint -> uint -> uint -> char ptr checkable

  (* Convert model to a string using the pretty printer.
   
   {v
   yices_model_to_string mdl n m p
   v}
